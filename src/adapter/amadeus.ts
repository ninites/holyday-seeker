...
class AmadeusAdapter {
  ...
  async getAccessToken() {
    try {
      ...
      console.log(
        `AMADEUS ADAPTER | getAccessToken | SUCCESS | access_token: ${access_token}`
      );
    } catch (error: any) {
      console.log(
        `AMADEUS ADAPTER | getAccessToken | ERROR | ${error.message}`
      );
      throw error;
    }
  }
...
const instance = new AmadeusAdapter();
export { instance, AmadeusAdapter };
